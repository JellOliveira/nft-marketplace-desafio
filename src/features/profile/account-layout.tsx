...
import { Download, Heart, LineChart, LogOut, MapPin, ShieldAlert, ShoppingCart, User as UserIcon } from 'lucide-react'
...
const OUT_OF_SCOPE_ITEMS = [
  ...
  { label: 'Ofertas', icon: LineChart },
  { label: 'Arquivos baixados', icon: Download },
  { label: 'Suporte', icon: ShieldAlert },
]
        ...
        <nav className="flex flex-col gap-1" aria-label="Navegação da conta">
          ...
          {OUT_OF_SCOPE_ITEMS.map(({ label, icon: Icon }) => (
            <span
              ...
              className="flex cursor-not-allowed items-center gap-3 rounded-md px-3 py-2 text-sm text-brand-text/50"
            >
              <Icon size={18} />
              {label}
            </span>
          ...
          <button
            ...
            className="mt-2 flex items-center gap-3 rounded-md border-t border-brand-border px-3 pt-3 text-sm font-bold text-brand-accent-alt"
          >
            <LogOut size={18} />
            Sair
          </button>
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        active
          ? 'border-brand-accent-alt bg-brand-elevated font-bold text-brand-accent-alt'
          : 'border-transparent text-brand-text hover:bg-brand-elevated',
      )}
    >
      <Icon size={18} />
      {label}
